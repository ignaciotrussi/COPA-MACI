  ...
  return (
    <div style={{ background: '#000', color: 'white', minHeight: '100vh', padding: '15px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{textAlign:'center', marginBottom:'15px'}}>
        <img src={logoMaci} alt="Logo" style={{width:'60px', borderRadius:'50%'}} />
      </div>
        ...
        <div style={{textAlign:'center'}}>
          ...
          {checkAdmin() && (
            <div style={{background:'#111', padding:'15px', borderRadius:'10px'}}>
              <input placeholder="Jugador RDM" onChange={e=>setGiraForm({...giraForm, jugador1: e.target.value})} />
              <input placeholder="Rival" onChange={e=>setGiraForm({...giraForm, rival: e.target.value})} />
              <button onClick={async () => {
                await fetch(`${API_URL}/gira`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(giraForm)});
                window.location.reload();
              }}>Guardar Match</button>
            </div>
          )}
        </div>
      )}
      ...
      {view === "cargar" && (
        <div style={{background:'#0a1a0a', padding:'20px', borderRadius:'15px', border:'1px solid #1b331b'}}>
          ...
          <button onClick={handleFinalize} style={{width:'100%', padding:'15px', marginTop:'20px', background:'#bbf7bb', color:'#000', fontWeight:'bold', border:'none', borderRadius:'10px'}}>FINALIZAR TARJETA</button>
        </div>
      )}

      {selectedPlayer && (
        <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'#000', padding:'20px', overflowY:'auto', zIndex:100}}>
          <button onClick={() => setSelectedPlayer(null)} style={btnStyle}>✕ Cerrar</button>
          ...
          <div style={{height: 250, width: '100%', marginTop:'20px'}}>
            <ResponsiveContainer>
              <LineChart data={getChartData(selectedPlayer.id)}>
                ...
                <Line type="monotone" dataKey="Mi_Neto" stroke="#bbf7bb" strokeWidth={3} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
